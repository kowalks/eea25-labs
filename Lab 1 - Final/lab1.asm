...
FIRST_PROMPT:
			...
			LDI		R16,'I'						; Op��o inicial padr�o � incremento
			STS		CARACTERE,R16
...
WAIT_SWITCH:								; Escuta por ativa��o do SWITCH
			IN		R16, PIND
			ANDI 	R16,0b10000000
			BRNE 	FOREVER
...
DECREMENTS:									; code for decrement
			...
			DEC		R20
OUTPUT:
			MOV		R21,R20		; R21 = R20<<2 (porque as portas PD0 e PD1 s�o usadas pelo terminal)
			LSL		R21
			LSL		R21
...
PRINT_STRING_LOOP:
			LPM		R16,Z+
			CPI		R16,'$'
			BREQ	PRINT_STRING_END		; Se n�o chegou no caractere de t�rmino '$', usa USART.
			CALL	USART_TRANSMIT
...
;  USART_LAZY_RECEIVE                      *
;  Subrotina                               *
;  Recebe os dados pela USART se h� algum  *
;  e retorna com o dado em R16.            *
;*******************************************
