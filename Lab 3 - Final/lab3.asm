...
TEST_SERVO_0:           ; Places (Z) em CONTx according to the servo specified by r22
   cpi   r22, '0'       ; Case servo = '0'
   brne  TEST_SERVO_1
   sts   CONTA+1, r26
   sts   CONTA, r27
   jmp   END_INPUT
TEST_SERVO_1:
   cpi   r22, '1'       ; Case servo = '1'
   brne  TEST_SERVO_2
   sts   CONTB+1, r26
   sts   CONTB, r27
   jmp   END_INPUT
TEST_SERVO_2:
   cpi   r22,'2'        ; Case servo = '2'
   brne  END_INPUT
   sts   CONTC+1, r26
   sts   CONTC, r27
END_INPUT:
   ldi   r20, 0         ; Set length = 0 to start new input
   ret
...
;************************************
;          LOOK UP TABLE            *
; para um ângulo a, foi caculado o  *
; valor round(100*(270+a)/9) - 1    *
; que corresponde à contagem para   *
; que o duty cycle coloque a largu- *
; ra de pulso necessária para posi- *
; cionar o servomotor no ângulo a   *
;************************************
   .org  0x200
POSL:
